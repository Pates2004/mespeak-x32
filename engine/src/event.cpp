...
ENTER("event_display");
	...
	else
	{
		...
		SHOW("event_display >   user_data=%p\n",(void*)event->user_data);
	}
#endif
}
//>
...
static void* pop()
{
	...
	if (head != NULL)
	{
		...
		free(n);
		node_counter--;
		SHOW("event > pop > event=%p (counter=%d, uid=%d)\n",(void*)the_data, node_counter,((espeak_EVENT*)the_data)->unique_identifier);
	}
