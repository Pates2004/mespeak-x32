...
static void init_buffer()
{
  ...
  SHOW("init_buffer > myRead=%p, myWrite=%p, BUFFER_LENGTH=0x%x, myReadPosition = myWritePosition = 0\n", (void*)myRead, (void*)myWrite, BUFFER_LENGTH);
}
...
#endif
{
	...
	else // myRead > aWrite
	{
		...
		{
			ptrdiff_t aTopMem = myBuffer + BUFFER_LENGTH - myRead;
			if (aTopMem)
			{
				SHOW("pa_callback > myRead=%p, aTopMem=0x%p\n",(void*)myRead, (void*)aTopMem);
				memcpy(outputBuffer, myRead, aTopMem);
			}
			ptrdiff_t aRest = n - aTopMem;
			if (aRest)
			{
				SHOW("pa_callback > myRead=%p, aRest=0x%p\n",(void*)myRead, (void*)aRest);
		...
		{ 
			...
			ptrdiff_t aRest = aWrite - myBuffer;
			if (aRest)
			{
				SHOW("pa_callback > myRead=%p, aRest=0x%p\n",(void*)myRead, (void*)aRest);
				char* p = (char*)outputBuffer + aTopMem;
